...
{
    if (!obj.contains("amount")   || !obj["amount"].isDouble())   return false;
    if (!obj.contains("type")     || !obj["type"].isString())     return false;
    if (!obj.contains("category") || !obj["category"].isString()) return false;
    if (!obj.contains("date")     || !obj["date"].isString())     return false;
...
{
    QJsonDocument doc(transactionToObject(t));
...
bool fromJson(const std::string& json, Transaction& out)
{
    ...
    QJsonDocument doc = QJsonDocument::fromJson(
        QByteArray::fromStdString(json), &error);
...
std::string extractAction(const std::string& json)
{
    ...
    if (!root.contains("action") || !root["action"].isString()) return "";
...
bool extractTransaction(const std::string& json, Transaction& out)
{
    ...
    QJsonObject root = doc.object();
    if (!root.contains("transaction") || !root["transaction"].isObject()) return false;

    return objectToTransaction(root["transaction"].toObject(), out);
}
